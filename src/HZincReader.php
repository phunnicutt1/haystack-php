 ...
 * - Replaced JavaScript's array indexing with PHP's array syntax
 * - Replaced JavaScript's string concatenation with PHP's string interpolation
 * - Replaced JavaScript's `switch` statements with PHP's `switch` statements
 * - Replaced JavaScript's `throw` statements with PHP's `throw` statements
 * - Replaced JavaScript's `function` declarations with PHP's class methods
 * - Replaced JavaScript's `var` with PHP's appropriate variable declarations
 * - Replaced JavaScript's `null` with PHP's `null`
 * - Replaced JavaScript's `undefined` with PHP's `null`
 * - Replaced JavaScript's `isNaN` with PHP's `is_nan`
 * - Replaced JavaScript's bitwise operations with PHP's bitwise operations
 * - Replaced JavaScript's `charCodeAt` with PHP's `ord`
 * - Replaced JavaScript's `String.fromCharCode` with PHP's `chr`
 ...
 * @return void
 */
function readGrid(HZincReader $self, callable $callback): void
{
    try {
        $b = new HGridBuilder();
