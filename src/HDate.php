...
class HDate extends HVal
{
    ...
    public function equals(HDate $that): bool
    {
        return $this->year === $that->year && $this->month === $that->month && $this->day === $that->day;
    }

    /** Return sort order as negative, 0, or positive */
    public function compareTo(HDate $that): int
    {
        return $this->year <=> $that->year ?: $this->month <=> $that->month ?: $this->day <=> $that->day;
    }
    ...
    public static function make($arg, ?int $month = null, ?int $day = null): HDate
    {
        if ($arg instanceof DateTime) {
            ...
            if ($arg < 1900) throw new InvalidArgumentException("Invalid year");
            if ($month < 1 || $month > 12) throw new InvalidArgumentException("Invalid month");
            if ($day < 1 || $day > 31) throw new InvalidArgumentException("Invalid day");
