...
class HTimeZone {
	...
	public static function make(string|DateTimeZone $arg1, bool $checked = FALSE) : ?HTimeZone
    {
        if (is_string($arg1)) {
			...
			if($arg1 === 'UTC')
			{
				...
				return new HTimeZone('UTC', $utc_timezone);
			}
	        ...
	        if (str_starts_with($tz_name, 'UTC'))
	        {
		        $utc_timezone = new DateTimeZone('UTC');
		        $utc_htimezone = new HTimeZone('UTC', $utc_timezone);
				return $utc_htimezone;
	        }
    ...
    public static function initialize(): void
    {
	    ...
	    $utc_timezone = new DateTimeZone('UTC');
        self::$UTC = self::make($utc_timezone, FALSE);

		$def = NULL;
	    $def = self::make('New_York', FALSE);

		if (self::$DEFAULT === NULL)
		{
	        //self::$UTC = self::make('UTC', FALSE);
	        self::$DEFAULT = self::make('New_York', FALSE);
           // $date = new \DateTime();
